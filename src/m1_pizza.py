...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(60.0, 50.0),  # All numbers are approximate.
                rg.Point(45.0, 58.7),
                rg.Point(45.0, 41.3)]
...
def run_test_draw_points_on_circle():
    ...
    title = 'MY TEST, test 4:  4 magenta dots.'
    window = rg.RoseWindow(600, 600, title)
    circle = rg.Circle(rg.Point(300, 300), 200)
    draw_points_on_circle(window, circle, 4, 'magenta')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        circle_guys = rg.Circle(points[k], 10)
        circle_guys.attach_to(window)
        circle_guys.fill_color = color
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(points)):
        ka_slice = rg.Line(points[k], circle.center)
        ka_slice.attach_to(window)
        ka_slice.color = color
        ka_slice.thickness = thickness
        window.render(1/24)
...
def run_test_polygon():
    ...
    window = rg.RoseWindow(650, 500, title)
    ...
    circle = rg.Circle(rg.Point(200, 400), 100)
    circle.outline_thickness = 4
    polygon(window, circle, 8, 'green', 7)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        if k < number_of_segments - 1:
            line_guy = rg.Line(points[k], points[k+1])
            line_guy.attach_to(window)
            line_guy.color = color
            line_guy.thickness = thickness
            window.render(1/24)
        else:
            line_guy = rg.Line(points[k], points[0])
            line_guy.attach_to(window)
            line_guy.color = color
            line_guy.thickness = thickness
            window.render(1 / 24)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(number_of_lines):
        line_guy = rg.Line(points[k], points[k - hops_to_next_point])
        line_guy.attach_to(window)
        line_guy.color = color
        line_guy.thickness = thickness
        window.render(1 / 24)
